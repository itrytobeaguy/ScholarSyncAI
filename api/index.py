...
def chat():
    ...
    if not message:
        return jsonify({"error": "Missing query message text parameter"}), 400
        
    ai_response = generate_chat_response(message, force=force, base64_image=base64_image, image_mime=image_mime, current_context=current_context)
    return jsonify({"result": ai_response})
...
def calendar_sync():
    ...
    if token == "mock_developer_access_token":
        return jsonify({"success": True, "calendar_status": "linked", "mock": True})
    ...
    try:
        ...
        if res.status_code in [200, 201]:
            return jsonify({"success": True, "calendar_status": "linked", "result": res.json()})
        else:
